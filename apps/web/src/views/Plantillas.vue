<template>
  <div class="p-6">
    ...
    <div class="space-y-4">
      <div v-for="tpl in plantillas" :key="tpl.id" class="panel-flat p-6">
        <div class="flex items-start justify-between">
          <div class="flex-1">
            ...
            <div class="flex flex-wrap gap-2 mt-2">
              <span
                ...
                :key="v"
                class="px-2 py-1 bg-zinc-100 dark:bg-zinc-700 text-zinc-600 dark:text-zinc-400 rounded-full text-xs"
                >{{ v }}</span
              >
            </div>
          </div>
          <div class="flex gap-2">
            <button
              @click="openModal(tpl)"
              class="px-4 py-2 border border-zinc-300 dark:border-zinc-600 text-zinc-600 dark:text-zinc-400 rounded-lg hover:bg-zinc-50 dark:hover:bg-zinc-700 text-sm"
            >
              Editar
            </button>
            <button
      ...
      <div v-if="cargando && !plantillas.length" class="panel-flat p-6">
        <Skeleton :filas="3" />
      </div>
      <div
        v-if="!cargando && plantillas.length === 0"
        class="panel-flat p-12 text-center"
      >
        ...
        <button
          @click="openModal(null)"
          class="bg-zinc-900 dark:bg-zinc-100 text-white dark:text-zinc-900 px-6 py-3 rounded-lg hover:opacity-90"
        >
    ...
    >
      <div
        class="bg-white dark:bg-zinc-800 rounded-2xl p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto"
      >
        ...
        <form @submit.prevent="guardarTemplate" class="space-y-4">
          ...
          <div>
            <label class="block text-sm mb-1"
              >Variables (separadas por coma)</label
            >
            <Input
