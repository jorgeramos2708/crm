<template>
  <div class="p-6">
    <div class="mb-4 flex flex-wrap items-center gap-2">
      <input v-model="q" @input="fetchProductos" type="text" placeholder="Buscar por nombre o SKU..." class="px-4 py-2 rounded-lg border border-zinc-300 dark:border-zinc-600 bg-white dark:bg-zinc-700 text-sm flex-1 min-w-[200px]" />
      <div class="ml-auto">
        <button @click="openModal(null)" class="bg-zinc-900 dark:bg-zinc-100 text-white dark:text-zinc-900 px-4 py-2 rounded-lg text-sm font-medium hover:opacity-90">+ Nuevo Producto</button>
      </div>
    </div>
        ...
        <table class="w-full">
          <thead class="bg-zinc-50 dark:bg-zinc-700/50">
            <tr>
              ...
              <th class="px-6 py-3 text-right text-xs font-medium text-zinc-500 uppercase tracking-wider">Precio</th>
          ...
          <tbody class="divide-y divide-zinc-200 dark:divide-zinc-700">
            <tr v-for="p in productos" :key="p.id" class="hover:bg-zinc-50 dark:hover:bg-zinc-700/50">
              <td class="px-6 py-4 text-sm font-medium">{{ p.nombre }}<div v-if="p.descripcion" class="font-normal text-xs text-zinc-500 truncate max-w-xs">{{ p.descripcion }}</div></td>
              <td class="px-6 py-4 text-sm text-zinc-500 font-mono text-xs">{{ p.sku || '—' }}</td>
              <td class="px-6 py-4 text-sm text-right tabular-nums">{{ formatCurrency(p.precio) }}</td>
              <td class="px-6 py-4 text-sm">{{ p.activo ? 'Activo' : 'Inactivo' }}</td>
              <td class="px-6 py-4 text-right text-sm">
                <button @click="openModal(p)" class="text-blue-600 hover:underline mr-3">Editar</button>
    ...
    <div v-if="showModal" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50">
      <div class="bg-white dark:bg-zinc-800 rounded-2xl p-6 w-full max-w-md">
        <h2 class="text-xl font-bold mb-4">{{ editing ? 'Editar Producto' : 'Nuevo Producto' }}</h2>
        <form @submit.prevent="guardar" class="space-y-4">
          ...
          <div class="grid grid-cols-2 gap-3">
            <div>
              ...
              <input v-model="form.sku" type="text" class="w-full px-4 py-2 rounded-lg border border-zinc-300 dark:border-zinc-600 bg-white dark:bg-zinc-700 font-mono text-sm" />
            </div>
            <div>
              <label class="block text-sm mb-1">Precio *</label>
              <input v-model.number="form.precio" type="number" min="0" required class="w-full px-4 py-2 rounded-lg border border-zinc-300 dark:border-zinc-600 bg-white dark:bg-zinc-700" />
            </div>
          </div>
          <div>
            <label class="block text-sm mb-1">Descripción</label>
            <textarea v-model="form.descripcion" rows="2" class="w-full px-4 py-2 rounded-lg border border-zinc-300 dark:border-zinc-600 bg-white dark:bg-zinc-700"></textarea>
          </div>
          <label class="flex items-center gap-2 text-sm cursor-pointer"><input type="checkbox" v-model="form.activo" class="w-4 h-4" /> Activo</label>
...
const openModal = (p) => {
  editing.value = p
  form.value = p
    ? { nombre: p.nombre, sku: p.sku || '', descripcion: p.descripcion || '', precio: p.precio, activo: p.activo }
...
const guardar = async () => {
  try {
    if (editing.value) await axios.put(`/api/productos/${editing.value.id}`, form.value)
