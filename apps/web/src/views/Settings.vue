<template>
  <div class="p-6 max-w-4xl">
    ...
    <div v-if="authStore.user?.role === 'admin'" class="panel-flat mb-3 overflow-hidden">
      <button @click="adminOpen = !adminOpen" class="flex w-full items-center gap-3 p-5 text-left">
        <span class="text-xl">🛡️</span>
        <span class="flex-1">
          <span class="block font-semibold">Administración</span>
          <span class="block text-xs text-zinc-400 dark:text-zinc-300">Usuarios, equipos y permisos</span>
        </span>
        <svg class="h-4 w-4 text-zinc-400 transition-transform" :class="adminOpen ? '' : '-rotate-90'" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M19 9l-7 7-7-7" /></svg>
      </button>
      <div v-show="adminOpen" class="px-5 pb-5 space-y-3">
    <!-- Usuarios y permisos (admin) -->
    <div v-if="authStore.user?.role === 'admin'" class="rounded-xl border border-zinc-200 dark:border-zinc-700 overflow-hidden">
      <button @click="acordeon = acordeon === 'usuarios' ? '' : 'usuarios'" class="flex w-full items-center gap-3 p-5 text-left">
        <span class="text-xl">👤</span>
        <span class="flex-1">
          <span class="block font-semibold">Usuarios</span>
          <span class="block text-xs text-zinc-500">Cuentas y permisos de lectura, escritura y borrado</span>
        </span>
        <svg class="h-4 w-4 text-zinc-400 transition-transform" :class="acordeon === 'usuarios' ? '' : '-rotate-90'" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M19 9l-7 7-7-7" /></svg>
      </button>
      <div v-show="acordeon === 'usuarios'" class="px-5 pb-5">
      <div class="flex items-center justify-end mb-4">
        <button @click="openUserModal(null)" class="bg-zinc-900 dark:bg-zinc-100 text-white dark:text-zinc-900 px-4 py-2 rounded-lg text-sm font-medium hover:opacity-90">+ Nuevo Usuario</button>
      </div>
        ...
        <li v-for="u in usuarios" :key="u.id" class="py-3">
          <div class="flex items-center justify-between">
            <div class="text-sm">
              ...
              <span class="text-zinc-500"> · {{ u.email }} · {{ u.role }}{{ u.activo ? '' : ' · inactivo' }}</span>
            </div>
            <div class="flex gap-2 text-xs">
              <button @click="editarPermisos(u)" class="text-blue-600 hover:underline">Permisos</button>
              <button @click="openUserModal(u)" class="text-blue-600 hover:underline">Editar</button>
              <button @click="borrarUsuario(u)" class="text-red-600 hover:underline">Eliminar</button>
            </div>
          </div>
          <div v-if="permUser?.id === u.id" class="mt-3 rounded-xl bg-zinc-50 dark:bg-zinc-900/50 p-3">
            <div class="grid grid-cols-[1fr_repeat(3,auto)] gap-x-4 gap-y-1.5 items-center text-sm">
              <span class="text-xs uppercase tracking-wide text-zinc-500">Módulo</span>
              <span class="text-xs font-medium text-center w-8" title="Leer">R</span>
              <span class="text-xs font-medium text-center w-8" title="Escribir">W</span>
              <span class="text-xs font-medium text-center w-8" title="Borrar">X</span>
              <template v-for="m in modulos" :key="m">
                <span>{{ m }}</span>
                <input type="checkbox" v-model="permMap[m].r" class="w-4 h-4 justify-self-center" />
                <input type="checkbox" v-model="permMap[m].w" class="w-4 h-4 justify-self-center" />
                <input type="checkbox" v-model="permMap[m].x" class="w-4 h-4 justify-self-center" />
              </template>
            </div>
            <div class="flex gap-2 mt-3">
              <button @click="guardarPermisos" class="px-4 py-1.5 rounded-lg bg-zinc-900 dark:bg-zinc-100 text-white dark:text-zinc-900 text-xs font-medium">Guardar permisos</button>
              <button @click="permUser = null" class="px-4 py-1.5 rounded-lg border border-zinc-300 dark:border-zinc-600 text-xs">Cerrar</button>
            </div>
          </div>
        </li>
        <li v-if="!usuarios.length" class="py-2 text-sm text-zinc-500">Sin usuarios.</li>
      </ul>
      </div>
    </div>
    <!-- Equipos (admin) -->
    <div v-if="authStore.user?.role === 'admin'" class="rounded-xl border border-zinc-200 dark:border-zinc-700 overflow-hidden">
      <button @click="acordeon = acordeon === 'equipos' ? '' : 'equipos'" class="flex w-full items-center gap-3 p-5 text-left">
        <span class="text-xl">👥</span>
        <span class="flex-1">
          <span class="block font-semibold">Equipos</span>
          <span class="block text-xs text-zinc-500">Agrupa usuarios para filtrar por alcance</span>
        </span>
        <svg class="h-4 w-4 text-zinc-400 transition-transform" :class="acordeon === 'equipos' ? '' : '-rotate-90'" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M19 9l-7 7-7-7" /></svg>
      </button>
    ...
    <div v-if="authStore.user?.role === 'admin'" class="panel-flat mb-3 overflow-hidden">
      <button @click="acordeon = acordeon === 'marca' ? '' : 'marca'" class="flex w-full items-center gap-3 p-5 text-left">
        <span class="text-xl">🎨</span>
        <span class="flex-1">
          <span class="block font-semibold">Personalización de CRM</span>
          <span class="block text-xs text-zinc-500">Tema, nombre, logo y colores de este despliegue</span>
        </span>
        <svg class="h-4 w-4 text-zinc-400 transition-transform" :class="acordeon === 'marca' ? '' : '-rotate-90'" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M19 9l-7 7-7-7" /></svg>
      </button>
      <div v-show="acordeon === 'marca'" class="px-5 pb-5">
        <form @submit.prevent="guardarMarca" class="space-y-4">
          <div>
            ...
            <input v-model="marcaForm.nombre" type="text" maxlength="60" placeholder="CRM" class="w-full px-4 py-2 rounded-lg border border-zinc-300 dark:border-zinc-600 bg-white dark:bg-zinc-700" />
          </div>
          <div>
            <label class="block text-sm mb-1">Tema general del CRM</label>
            <div class="flex flex-wrap gap-2 mb-2">
              <button v-for="t in temasPreset" :key="t.nombre" type="button" @click="aplicarTema(t)" class="flex items-center gap-2 rounded-xl border-2 px-3 py-2 text-xs" :class="temaActivo(t) ? 'border-zinc-900 dark:border-zinc-100' : 'border-zinc-200 dark:border-zinc-700'" :title="t.nombre">
                <span class="flex -space-x-1">
                  <span class="h-5 w-5 rounded-full border border-zinc-300" :style="{ backgroundColor: t.fondo }"></span>
                  <span class="h-5 w-5 rounded-full border border-zinc-300" :style="{ backgroundColor: t.panel }"></span>
                  <span class="h-5 w-5 rounded-full border border-zinc-300" :style="{ backgroundColor: t.primario }"></span>
                </span>
                {{ t.nombre }}
              </button>
            </div>
            <div class="grid grid-cols-2 gap-3">
              <div class="flex items-center gap-2">
                <input v-model="marcaForm.temaFondo" type="color" class="h-9 w-12 cursor-pointer rounded-lg border border-zinc-300 dark:border-zinc-600 bg-white dark:bg-zinc-700 p-1" />
                <span class="text-xs text-zinc-500">Fondo</span>
              </div>
              <div class="flex items-center gap-2">
                <input v-model="marcaForm.temaPanel" type="color" class="h-9 w-12 cursor-pointer rounded-lg border border-zinc-300 dark:border-zinc-600 bg-white dark:bg-zinc-700 p-1" />
                <span class="text-xs text-zinc-500">Paneles</span>
              </div>
            </div>
          </div>
          <div>
            <label class="block text-sm mb-1">Color de los módulos del menú</label>
            <div class="flex items-center gap-3">
              <input v-model="marcaForm.color" type="color" class="h-10 w-14 cursor-pointer rounded-lg border border-zinc-300 dark:border-zinc-600 bg-white dark:bg-zinc-700 p-1" />
              <div class="flex gap-1.5">
                <button v-for="c in ['#1c1917', '#0F6CBD', '#047857', '#B45309', '#9D174D']" :key="c" type="button" @click="marcaForm.color = c" class="h-7 w-7 rounded-full border border-zinc-300" :style="{ backgroundColor: c }" :title="c"></button>
              </div>
              <button type="button" @click="marcaForm.color = ''" class="text-xs text-zinc-500 hover:underline">Restablecer</button>
            </div>
          </div>
          <div>
            <label class="block text-sm mb-1">Logo (PNG/JPG, máx. 500 KB)</label>
            <div class="flex items-center gap-3">
              <img v-if="marcaLogoPreview" :src="marcaLogoPreview" alt="logo" class="h-12 w-12 rounded-xl object-cover border border-zinc-200 dark:border-zinc-700" />
              <label class="px-4 py-2 rounded-lg border border-zinc-300 dark:border-zinc-600 text-sm cursor-pointer hover:bg-zinc-50 dark:hover:bg-zinc-700">
                Elegir archivo
                <input type="file" accept="image/png,image/jpeg,image/webp" class="hidden" @change="elegirLogo" />
              </label>
              <button v-if="marcaLogoPreview" type="button" @click="quitarLogo" class="text-xs text-red-600 hover:underline">Quitar</button>
            </div>
          </div>
          <div>
            <label class="block text-sm mb-1">Fondo del inicio de sesión</label>
            <div class="flex flex-wrap items-center gap-2">
              <button v-for="f in fondosPreset" :key="f.valor || 'def'" type="button" @click="marcaForm.fondo = f.valor; marcaFondoPreview = ''" class="h-10 w-16 rounded-lg border-2 overflow-hidden" :class="marcaForm.fondo === f.valor && !marcaFondoPreview ? 'border-zinc-900 dark:border-zinc-100' : 'border-zinc-200 dark:border-zinc-700'" :title="f.nombre">
                <span class="block h-full w-full" :style="f.estilo"></span>
              </button>
              <label class="px-4 py-2 rounded-lg border border-zinc-300 dark:border-zinc-600 text-sm cursor-pointer hover:bg-zinc-50 dark:hover:bg-zinc-700">
                Subir imagen
                <input type="file" accept="image/png,image/jpeg,image/webp" class="hidden" @change="elegirFondo" />
              </label>
              <button v-if="fondoActualPreview" type="button" @click="quitarFondo" class="text-xs text-red-600 hover:underline">Restablecer</button>
            </div>
          </div>
          <div class="flex items-center gap-2">
            ...
            <button type="button" @click="showPreview = true" class="px-4 py-2 rounded-lg border border-zinc-300 dark:border-zinc-600 text-sm hover:bg-zinc-50 dark:hover:bg-zinc-700">Previsualización</button>
            <span v-if="marcaMsg" class="text-sm text-zinc-500">{{ marcaMsg }}</span>
          </div>
        </form>
      </div>
    </div>
    ...
    <div v-if="showPreview" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" @click.self="showPreview = false">
      <div class="w-full max-w-sm overflow-hidden rounded-2xl">
        <div class="relative flex min-h-[480px] items-center justify-center p-6" :style="previewFondoStyle">
          <div class="absolute inset-0 bg-white/40"></div>
          <div class="relative w-full rounded-2xl bg-white p-6 shadow-xl">
            <div class="mb-4 text-center">
              <img v-if="marcaLogoPreview" :src="marcaLogoPreview" alt="logo" class="mx-auto mb-2 h-10 w-10 rounded-xl object-cover" />
              <p class="text-xs font-semibold tracking-[0.2em] text-zinc-500">{{ previewNombre }}</p>
              <h3 class="mt-1 text-xl font-bold">Inicio de Sesión</h3>
            </div>
            <div class="space-y-3">
              <div class="rounded-lg border border-zinc-300 px-4 py-2.5 text-sm text-zinc-400">nombre@empresa.com</div>
              <div class="rounded-lg border border-zinc-300 px-4 py-2.5 text-sm text-zinc-400">••••••••••</div>
              <div class="rounded-lg px-4 py-2.5 text-center text-sm font-medium text-white" :style="previewBtnStyle" :class="{ 'bg-zinc-900': !marcaForm.color }">Iniciar Sesión</div>
            </div>
            <p class="mt-3 text-center text-[11px] text-zinc-400">Vista previa sin guardar</p>
          </div>
        </div>
        <button @click="showPreview = false" class="w-full bg-zinc-900 px-4 py-2.5 text-sm font-medium text-white">Cerrar</button>
      </div>
    </div>

    <div class="panel-flat mb-3 overflow-hidden">
      <button @click="acordeon = acordeon === 'conexiones' ? '' : 'conexiones'" class="flex w-full items-center gap-3 p-5 text-left">
        <span class="text-xl">🔌</span>
        <span class="flex-1">
          <span class="block font-semibold">Conexiones</span>
          <span class="block text-xs text-zinc-500">Buzones de Outlook y Gmail</span>
        </span>
        <svg class="h-4 w-4 text-zinc-400 transition-transform" :class="acordeon === 'conexiones' ? '' : '-rotate-90'" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M19 9l-7 7-7-7" /></svg>
      </button>
    ...
    <div v-if="authStore.user?.role === 'admin'" class="panel-flat mb-3 overflow-hidden">
      <button @click="acordeon = acordeon === 'credenciales' ? '' : 'credenciales'" class="flex w-full items-center gap-3 p-5 text-left">
        <span class="text-xl">🔑</span>
        <span class="flex-1">
          <span class="block font-semibold">Credenciales</span>
          <span class="block text-xs text-zinc-500">Client ID y Secret de Microsoft y Google</span>
        </span>
        <svg class="h-4 w-4 text-zinc-400 transition-transform" :class="acordeon === 'credenciales' ? '' : '-rotate-90'" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M19 9l-7 7-7-7" /></svg>
      </button>
    ...
    <div class="rounded-xl border border-zinc-200 dark:border-zinc-700 p-4">
      ...
      <div class="text-sm text-zinc-500 mb-4 space-y-1">
        <p class="font-medium text-zinc-600 dark:text-zinc-400">Cómo obtener el Client ID y el Secret:</p>
        <ol class="list-decimal ml-5 space-y-0.5">
          <li>Entra a <span class="font-mono text-xs">console.cloud.google.com</span> → APIs y servicios → Credenciales.</li>
          <li>Crear credenciales → ID de cliente OAuth → tipo <b>Web</b>.</li>
          <li>En "URI de redirección" agrega la de abajo y guarda.</li>
          <li>Copia el ID y el secreto y pégalos aquí. Si la app está en modo prueba, añade a los empleados como usuarios de prueba.</li>
        </ol>
      </div>
      ...
      <form @submit.prevent="guardarGoogleConfig" class="space-y-4">
        ...
        <div>
          ...
          <textarea v-model="gScopesText" rows="5" placeholder="https://www.googleapis.com/auth/gmail.readonly&#10;https://www.googleapis.com/auth/gmail.send&#10;https://www.googleapis.com/auth/calendar.readonly&#10;https://www.googleapis.com/auth/userinfo.email&#10;openid" class="w-full px-4 py-2 rounded-lg border border-zinc-300 dark:border-zinc-600 bg-white dark:bg-zinc-700 font-mono text-sm"></textarea>
          <p class="text-xs text-zinc-500 mt-1">Si se dejan vacíos se usan los recomendados. Si agregas uno nuevo, cada empleado debe reconectar su cuenta.</p>
        </div>
        <div class="flex items-center gap-2">
          <button type="submit" :disabled="saving" class="bg-zinc-900 dark:bg-zinc-100 text-white dark:text-zinc-900 px-4 py-2 rounded-lg text-sm font-medium hover:opacity-90 disabled:opacity-50">{{ saving ? 'Guardando...' : 'Guardar' }}</button>
    ...
    <div class="rounded-xl border border-zinc-200 dark:border-zinc-700 p-4">
      ...
      <div class="text-sm text-zinc-500 mb-4 space-y-1">
        <p class="font-medium text-zinc-600 dark:text-zinc-400">Cómo obtener el Client ID y el Secret:</p>
        <ol class="list-decimal ml-5 space-y-0.5">
          <li>Entra a <span class="font-mono text-xs">entra.microsoft.com</span> → Aplicaciones → Registros → Nuevo registro (single-tenant).</li>
          <li>Copia el <b>ID de aplicación (cliente)</b> y créale un <b>secreto de cliente</b> en Certificados y secretos.</li>
          <li>En Permisos de API agrega los delegados de la lista de abajo y otorga consentimiento de administrador.</li>
          <li>En Autenticación agrega la URI de redirección de abajo. Los empleados solo usan "Conectar Outlook".</li>
        </ol>
      </div>
      ...
      <form @submit.prevent="guardarConfig" class="space-y-4">
        ...
        <div>
          <label class="block text-sm mb-1">Scopes (uno por línea)</label>
          <textarea v-model="scopesText" rows="5" placeholder="https://graph.microsoft.com/Mail.Read&#10;https://graph.microsoft.com/Mail.Send&#10;https://graph.microsoft.com/Calendars.Read&#10;https://graph.microsoft.com/User.Read&#10;offline_access" class="w-full px-4 py-2 rounded-lg border border-zinc-300 dark:border-zinc-600 bg-white dark:bg-zinc-700 font-mono text-sm"></textarea>
          <p class="text-xs text-zinc-500 mt-1">Definen a qué puede acceder el CRM en cada buzón. Deben coincidir con los permisos del App Registration; si agregas uno nuevo, cada empleado debe reconectar su cuenta.</p>
        </div>
        <div class="flex items-center gap-2">
          <button type="submit" :disabled="saving" class="bg-zinc-900 dark:bg-zinc-100 text-white dark:text-zinc-900 px-4 py-2 rounded-lg text-sm font-medium hover:opacity-90 disabled:opacity-50">{{ saving ? 'Guardando...' : 'Guardar' }}</button>
    ...
    <div v-if="authStore.user?.role === 'admin'" class="panel-flat mb-3 overflow-hidden">
      <button @click="acordeon = acordeon === 'campos' ? '' : 'campos'" class="flex w-full items-center gap-3 p-5 text-left">
        <span class="text-xl">📝</span>
        <span class="flex-1">
          <span class="block font-semibold">Campos</span>
          <span class="block text-xs text-zinc-500">Campos propios de contactos, empresas y oportunidades</span>
        </span>
        <svg class="h-4 w-4 text-zinc-400 transition-transform" :class="acordeon === 'campos' ? '' : '-rotate-90'" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M19 9l-7 7-7-7" /></svg>
      </button>
    ...
    <div v-if="showUserModal" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50">
      <div class="bg-white dark:bg-zinc-800 rounded-2xl p-6 w-full max-w-md">
        <h2 class="text-xl font-bold mb-4">{{ editingUser ? 'Editar Usuario' : 'Nuevo Usuario' }}</h2>
        <form @submit.prevent="guardarUsuario" class="space-y-4">
          <div>
            <label class="block text-sm mb-1">Nombre *</label>
            <input v-model="userForm.name" type="text" required class="w-full px-4 py-2 rounded-lg border border-zinc-300 dark:border-zinc-600 bg-white dark:bg-zinc-700" />
          </div>
          <div>
            <label class="block text-sm mb-1">Email *</label>
            <input v-model="userForm.email" type="email" required :disabled="!!editingUser" class="w-full px-4 py-2 rounded-lg border border-zinc-300 dark:border-zinc-600 bg-white dark:bg-zinc-700 disabled:opacity-50" />
          </div>
          <div>
            <label class="block text-sm mb-1">{{ editingUser ? 'Nueva contraseña (vacío = mantener)' : 'Contraseña *' }}</label>
            <input v-model="userForm.password" type="password" :required="!editingUser" autocomplete="new-password" class="w-full px-4 py-2 rounded-lg border border-zinc-300 dark:border-zinc-600 bg-white dark:bg-zinc-700" />
          </div>
          <div class="grid grid-cols-2 gap-3">
            <div>
              <label class="block text-sm mb-1">Rol</label>
              <select v-model="userForm.role" class="w-full px-4 py-2 rounded-lg border border-zinc-300 dark:border-zinc-600 bg-white dark:bg-zinc-700">
                <option value="user">Usuario</option>
                <option value="admin">Admin</option>
              </select>
            </div>
            <label class="flex items-center gap-2 text-sm pt-6 cursor-pointer"><input type="checkbox" v-model="userForm.activo" class="w-4 h-4" /> Activo</label>
          </div>
          <div class="flex gap-2 pt-2">
            <button type="button" @click="showUserModal = false" class="flex-1 px-4 py-2 rounded-lg border border-zinc-300 dark:border-zinc-600">Cancelar</button>
    ...
    <!-- Integraciones -->
    <div class="panel-flat mb-3 overflow-hidden">
      <button @click="acordeon = acordeon === 'integraciones' ? '' : 'integraciones'" class="flex w-full items-center gap-3 p-5 text-left">
        <span class="text-xl">🧩</span>
        <span class="flex-1">
          <span class="block font-semibold">Integraciones</span>
          <span class="block text-xs text-zinc-500">Tokens de API y webhooks salientes</span>
        </span>
        <svg class="h-4 w-4 text-zinc-400 transition-transform" :class="acordeon === 'integraciones' ? '' : '-rotate-90'" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M19 9l-7 7-7-7" /></svg>
      </button>
    ...
    <div v-if="authStore.user?.role === 'admin'" class="rounded-xl border border-zinc-200 dark:border-zinc-700 p-4">
...
const marcaForm = ref({ nombre: '', color: '', logo: '', fondo: '', temaFondo: '#faf9f7', temaPanel: '#f1eee7' })
const marcaLogoPreview = ref('')
const marcaFondoPreview = ref('')
const marcaMsg = ref('')
const showPreview = ref(false)
const temasPreset = [
  { nombre: 'Alabastro', fondo: '#faf9f7', panel: '#f1eee7', primario: '#1c1917' },
  ...
  { nombre: 'Grafito', fondo: '#f4f4f5', panel: '#e2e2e6', primario: '#3f3f46' },
]
const aplicarTema = (t) => {
...
const temaActivo = (t) => marcaForm.value.temaFondo === t.fondo && marcaForm.value.temaPanel === t.panel
const fondosPreset = [
  { nombre: 'Por defecto', valor: '', estilo: 'background: linear-gradient(135deg, #e9e4d9, #d7d0c2)' },
...
const fondoActualPreview = computed(() => marcaFondoPreview.value || marcaForm.value.fondo)
const previewFondoStyle = computed(() => {
  const f = marcaFondoPreview.value || marcaForm.value.fondo
  if (f && f.startsWith('data:')) return { backgroundImage: `url(${f})`, backgroundSize: 'cover', backgroundPosition: 'center' }
  if (f) return { backgroundColor: f }
  return { background: 'linear-gradient(135deg, #e9e4d9, #d7d0c2)' }
})
const previewBtnStyle = computed(() => marcaForm.value.color ? { backgroundColor: marcaForm.value.color } : {})
...
const guardarMarca = async () => {
  ...
  try {
    const payload = {
      ...
      tema: { fondo: marcaForm.value.temaFondo, panel: marcaForm.value.temaPanel },
    }
    payload.logo = marcaForm.value.logo === '__quitar__' ? '' : (marcaForm.value.logo || undefined)
    ...
    const { loadMarca: reload, marca: marcaActual } = await import('../utils/marca.js')
    await reload()
    marcaForm.value = { nombre: marcaActual.value.nombre === 'CRM' ? '' : marcaActual.value.nombre, color: marcaActual.value.color, logo: '', fondo: marcaActual.value.fondo, temaFondo: marcaActual.value.tema.fondo, temaPanel: marcaActual.value.tema.panel }
...
const borrarCustomField = async (f) => {
  if (!await confirmar(`¿Eliminar campo ${f.etiqueta}? Los valores guardados se conservan.`)) return
...
const modulos = ['contactos', 'empresas', 'oportunidades', 'productos', 'presupuestos', 'tareas', 'correos', 'calendario', 'campañas', 'plantillas', 'listas', 'automatizaciones', 'actividades', 'reportes']
const showUserModal = ref(false)
const editingUser = ref(null)
const userForm = ref({ name: '', email: '', password: '', role: 'user', activo: true })
const permUser = ref(null)
const permMap = ref({})

const emptyPermMap = () => Object.fromEntries(modulos.map(m => [m, { r: false, w: false, x: false }]))
...
const openUserModal = (u) => {
  editingUser.value = u
  userForm.value = u
    ? { name: u.name, email: u.email, password: '', role: u.role, activo: u.activo }
...
const guardarUsuario = async () => {
  try {
    if (editingUser.value) {
      const payload = { name: userForm.value.name, role: userForm.value.role, activo: userForm.value.activo }
    ...
    await fetchUsuarios()
  } catch (e) { toast.error(e.response?.data?.error || 'Error guardando usuario') }
}
...
const guardarPermisos = async () => {
  try {
    ...
    permUser.value = null
  } catch (e) { toast.error(e.response?.data?.error || 'Error guardando permisos') }
}
...
const crearWebhook = async () => {
  try {
    ...
    await fetchWebhooks()
  } catch (e) { toast.error(e.response?.data?.error || 'Error creando webhook') }
}
...
const guardarGoogleConfig = async () => {
  ...
  try {
    const payload = {
      ...
      scopes: gScopesText.value.split('\n').map(s => s.trim()).filter(Boolean),
    }
...
onMounted(async () => {
  ...
  try {
    ...
    marcaForm.value = { nombre: data.nombre === 'CRM' ? '' : (data.nombre || ''), color: data.color || '', logo: '', fondo: data.fondo || '', temaFondo: (data.tema || {}).fondo || '#faf9f7', temaPanel: (data.tema || {}).panel || '#f1eee7' }
