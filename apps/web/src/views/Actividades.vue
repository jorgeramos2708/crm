  ...
  <div class="p-6">
    <div class="bg-white dark:bg-zinc-800 rounded-2xl shadow-sm border border-zinc-200 dark:border-zinc-700 overflow-hidden">
      <div class="p-4 border-b border-zinc-200 dark:border-zinc-700 flex flex-wrap gap-4">
        ...
        <select v-model="filtroEntidad" @change="resetAndFetch" class="px-4 py-2 border border-zinc-300 dark:border-zinc-600 rounded-lg bg-white dark:bg-zinc-700 text-sm">
        ...
        <table class="w-full">
          <thead class="bg-zinc-50 dark:bg-zinc-700/50">
            <tr>
              ...
              <th class="px-6 py-3 text-left text-xs font-medium text-zinc-500 dark:text-zinc-400 uppercase tracking-wider">Entidad</th>
          ...
          <tbody class="divide-y divide-zinc-200 dark:divide-zinc-700">
            <tr v-for="act in actividades" :key="act.id" class="hover:bg-zinc-50 dark:hover:bg-zinc-700/50">
              <td class="px-6 py-4 whitespace-nowrap text-sm text-zinc-500 dark:text-zinc-400">{{ formatDate(act.createdAt) }}</td>
              <td class="px-6 py-4 whitespace-nowrap">
                <span :class="getTipoClass(act.tipo)" class="px-2 py-1 rounded-full text-xs font-medium">{{ act.tipo }}</span>
              </td>
              <td class="px-6 py-4 text-sm font-medium text-zinc-900 dark:text-zinc-100">{{ act.titulo }}</td>
...
const getTipoClass = (tipo) => {
  const classes = {
    ...
    task_created: 'bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-400',
    notification: 'bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-400',
    stage_changed: 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400'
  }
