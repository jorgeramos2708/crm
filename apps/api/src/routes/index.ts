...
function stripReadonly<T extends Record<string, any>>(
  body: T,
...
export async function registerAuthRoutes(app: FastifyInstance) {
  ...
  app.get("/api/ajustes/marca", async () => {
    const rows = await db
      .select()
      .from(ajustes)
      .where(or(...MARCA_CLAVES.map((c) => eq(ajustes.clave, c))));
    const map: Record<string, any> = {};
    for (const r of rows) map[r.clave] = r.valor;
    const tema = (
      map.marca_tema && typeof map.marca_tema === "object" ? map.marca_tema : {}
    ...
    return {
      ...
      tema: {
        fondo: /^#[0-9a-fA-F]{6}$/.test(tema.fondo || "")
          ? tema.fondo
          : TEMA_DEFAULT.fondo,
        panel: /^#[0-9a-fA-F]{6}$/.test(tema.panel || "")
          ? tema.panel
          : TEMA_DEFAULT.panel,
      },
    };
  ...
  app.put(
    ...
    async (req, res) => {
      const body = req.body as {
        ...
        logo?: string | null;
        fondo?: string | null;
        tema?: { fondo?: string; panel?: string };
      };
      ...
      if (body.nombre !== undefined) {
        ...
        await db
          ...
          .onConflictDoUpdate({
            ...
            set: { valor: nombre, updatedAt: new Date() },
          });
        out.nombre = nombre;
      }
      if (body.color !== undefined) {
        ...
        await db
          .insert(ajustes)
          .values({ clave: "marca_color", valor: color, updatedAt: new Date() })
          .onConflictDoUpdate({
            target: [ajustes.clave],
            set: { valor: color, updatedAt: new Date() },
          });
        out.color = color;
      }
      if (body.logo !== undefined) {
        const logo =
          body.logo &&
          String(body.logo).startsWith("data:image/") &&
          String(body.logo).length <= 500000
            ? String(body.logo)
            : "";
        await db
          .insert(ajustes)
          .values({ clave: "marca_logo", valor: logo, updatedAt: new Date() })
          .onConflictDoUpdate({
            target: [ajustes.clave],
            set: { valor: logo, updatedAt: new Date() },
          });
        out.logo = logo ? "ok" : "";
      }
      if (body.fondo !== undefined) {
        ...
        const fondo =
          f === "" ||
          /^#[0-9a-fA-F]{6}$/.test(f) ||
          (f.startsWith("data:image/") && f.length <= 1000000)
            ? f
            : "";
        await db
          .insert(ajustes)
          .values({ clave: "marca_fondo", valor: fondo, updatedAt: new Date() })
          .onConflictDoUpdate({
            target: [ajustes.clave],
            set: { valor: fondo, updatedAt: new Date() },
          });
        out.fondo = fondo ? "ok" : "";
      }
      if (body.tema !== undefined && typeof body.tema === "object") {
        ...
        const tema = {
          fondo: esHex(body.tema?.fondo)
            ? String(body.tema.fondo)
            : TEMA_DEFAULT.fondo,
          panel: esHex(body.tema?.panel)
            ? String(body.tema.panel)
            : TEMA_DEFAULT.panel,
        };
        await db
          .insert(ajustes)
          ...
          .onConflictDoUpdate({
            target: [ajustes.clave],
            set: { valor: tema as any, updatedAt: new Date() },
          });
        out.tema = "ok";
      }
      return out;
  ...
  const PERM_RULES: Array<{
    ...
    {
      method: "GET",
      re: /^\/api\/(microsoft|google)\/(emails|profile|subscriptions|status)(\/|$)/,
      modulo: "correos",
      accion: "r",
    },
    {
      method: "POST",
      re: /^\/api\/(microsoft|google)\/send(\/|$)/,
      modulo: "correos",
      accion: "w",
    },
    {
      method: "POST",
      re: /^\/api\/microsoft\/vincular(\/|$)/,
      modulo: "correos",
      accion: "w",
    },
    {
  ...
  app.addHook("preHandler", async (req, reply) => {
    ...
    const rule = PERM_RULES.find(
      (r) => r.method === req.method && r.re.test(path),
    );
    if (!rule) return;
    ...
    if (!ok) {
      return reply
        .code(403)
        .send({ error: "Sin permiso para este módulo", code: "FORBIDDEN" });
    }
  });
  ...
  app.post(
    ...
    async (req, res) => {
      ...
      if (!user.activo) {
        return res
          .code(403)
          .send({ error: "Usuario inactivo", code: "USER_INACTIVE" });
      }
  ...
  app.get(
    "/api/email-subscriptions/confirm/:token",
    async (req: FastifyRequest<{ Params: EmailSubscriptionParams }>, res) => {
      const [sub] = await db
        .select()
        .from(emailSubscriptions)
        .where(eq(emailSubscriptions.token, req.params.token))
        .limit(1);
      if (!sub) return res.code(404).send({ error: "Token inválido" });
      if (sub.estado === "confirmado")
        return res.code(400).send({ error: "Ya confirmado" });
      if (sub.estado === "cancelado")
        return res.code(400).send({ error: "Suscripción cancelada" });
      await db
        .update(emailSubscriptions)
        .set({ estado: "confirmado", confirmadoEn: new Date() })
  ...
  app.get(
    "/api/email-subscriptions/unsubscribe/:token",
    async (req: FastifyRequest<{ Params: EmailSubscriptionParams }>, res) => {
      const [sub] = await db
        .select()
        .from(emailSubscriptions)
  ...
  app.get(
    ...
    async (
      req: FastifyRequest<{
        Querystring: { contactoId?: string; listaId?: string; estado?: string };
      }>,
      res,
    ) => {
      ...
      const conditions = [];
      if (contactoId)
        conditions.push(eq(emailSubscriptions.contactoId, contactoId));
  ...
  app.get(
    "/api/graph/notify",
    async (
      req: FastifyRequest<{ Querystring: { validationToken?: string } }>,
      res,
    ) => {
    ...
    async (
      ...
      for (const n of body?.value || []) {
        ...
        const [sub] = await db
          ...
          .where(
            eq(microsoftGraphSubscriptions.subscriptionId, n.subscriptionId),
          )
          .limit(1);
        ...
        if (
          sub.clientState &&
          n.clientState &&
          sub.clientState !== n.clientState
        ) {
          console.warn(
            `Graph notify clientState inválido para ${n.subscriptionId}`,
          );
          continue;
        }
        await redis.xadd(
  ...
  app.get("/api/auth/:proveedor/login", async (req, res) => {
    ...
    const state = Buffer.from(
      JSON.stringify({ purpose: "login", nonce }),
    ).toString("base64url");
    const url =
      proveedor === "google"
        ? await getGoogleAuthUrl(state)
  ...
  app.get(
    "/api/auth/:proveedor/login/callback",
    async (
      req: FastifyRequest<{
      ...
      try {
        ...
        return fail("sso_invalido");
      }
      if (parsed?.purpose !== "login" || !parsed?.nonce)
        return fail("sso_invalido");
  ...
  app.get(
    ...
    async (
      ...
      if (error) {
        return res.redirect(
          `${process.env.FRONTEND_URL || "http://localhost:8081"}/settings?microsoft_error=${error}`,
        );
      }
      if (!code) {
        return res
          .code(400)
          .send({ error: "Código de autorización requerido" });
      }
      if (!state) {
      ...
      try {
        ...
        if (!config) {
          return res.redirect(
            `${process.env.FRONTEND_URL || "http://localhost:8081"}/settings?microsoft_error=config_not_found`,
          );
        }
        const tokens = await exchangeCodeForTokens(config, code);
        await storeUserTokens(parsedState.userId, tokens);
        return res.redirect(
          `${process.env.FRONTEND_URL || "http://localhost:8081"}/settings?microsoft_connected=true`,
        );
      } catch (err) {
        console.error("Microsoft OAuth error:", err);
        return res.redirect(
          `${process.env.FRONTEND_URL || "http://localhost:8081"}/settings?microsoft_error=token_exchange_failed`,
        );
      }
    },
  );
  ...
  app.register(async (app) => {
      ...
      async (
        ...
        try {
          ...
          if (contactoId) {
            await logActivity(
              "email_sent",
              `Email Outlook: ${email.subject}`,
              "contacto",
              contactoId,
              req.user!.id,
              { via: "microsoft-graph", to: email.to },
            );
          }
          if (oportunidadId) {
            await logActivity(
    ...
    app.post("/api/microsoft/vincular", async (req, res) => {
      const { messageId, asunto, from, contactoId, oportunidadId } =
        req.body as {
          messageId?: string;
          asunto?: string;
          from?: string;
      ...
      if (!contactoId && !oportunidadId) {
        return res
          .code(400)
          .send({ error: "contactoId u oportunidadId requerido" });
      }
    ...
    app.get(
      ...
      async (
        ...
        try {
          ...
          const emails = await getUserEmails(req.user!.id, {
            folder,
            top: parseInt(top),
            skip: parseInt(skip),
            filter,
      ...
      async (
        ...
        try {
          ...
          const notificationUrl =
            ...
            `${env.GRAPH_PUBLIC_URL}/api/graph/notify`;
          const subId = await createUserSubscription(
            req.user!.id,
            resource,
            changeType,
            notificationUrl,
    ...
    app.post(
      "/api/microsoft/subscriptions/:id/renew",
      async (req: FastifyRequest<{ Params: { id: string } }>, res) => {
        const ok = await renewUserSubscription(req.params.id);
        if (!ok)
          return res.code(500).send({ error: "Error renovando suscripción" });
        return { ok: true };
      },
    );

    app.delete(
      "/api/microsoft/subscriptions/:id",
      async (req: FastifyRequest<{ Params: { id: string } }>, res) => {
        const ok = await deleteUserSubscription(req.params.id);
        if (!ok)
          return res.code(500).send({ error: "Error eliminando suscripción" });
        return { ok: true };
      },
    );
    ...
    app.get("/api/microsoft/subscriptions", async (req, res) => {
      const subs = await db
        .select()
        .from(microsoftGraphSubscriptions)
        .where(eq(microsoftGraphSubscriptions.userId, req.user!.id));
      return subs;
    });
    ...
    app.get("/api/microsoft/status", async (req, res) => {
      const [token] = await db
        .select()
        .from(microsoftGraphTokens)
        .where(eq(microsoftGraphTokens.userId, req.user!.id))
        .limit(1);
    ...
    app.delete("/api/microsoft/disconnect", async (req, res) => {
      await db
        .delete(microsoftGraphTokens)
        .where(eq(microsoftGraphTokens.userId, req.user!.id));
      await db
        .delete(microsoftGraphSubscriptions)
        .where(eq(microsoftGraphSubscriptions.userId, req.user!.id));
      return { ok: true };
    });
    ...
    app.get(
      ...
      async () => {
        const [config] = await db
          ...
          .where(eq(microsoftConfig.activo, true))
          .limit(1);
        if (!config) return { configured: false };
        const { clientSecret, ...safe } = config;
        return {
          configured: true,
          config: { ...safe, clientSecretConfigured: !!clientSecret },
        };
      },
    );
    ...
    app.put(
      ...
      async (req, res) => {
        ...
        if (!body.clientId || !body.redirectUri) {
          return res
            .code(400)
            .send({ error: "clientId y redirectUri son requeridos" });
        }
        ...
        const [updated] = await db
          ...
          .where(eq(microsoftConfig.id, existing.id))
          .returning();
    ...
    app.get(
      ...
      async (req, res) => {
        ...
        if (!authUrl)
          return res.code(400).send({ error: "Google no configurado" });
        res.redirect(authUrl);
      },
    );
    ...
    app.get(
      ...
      async (
        ...
        if (!code || !state)
          return res.code(400).send({ error: "code y state requeridos" });
        ...
        try {
          const cfg = await getGoogleConfig();
          if (!cfg)
            return res.redirect(`${FE}/settings?google_error=config_not_found`);
          const tokens = await exchangeGoogleCode(cfg, code);
          ...
          return res.redirect(
            `${FE}/settings?google_error=token_exchange_failed`,
          );
        }
      },
    );
    ...
    app.get("/api/google/status", async (req) => {
      const [t] = await db
        .select()
        .from(googleTokens)
        .where(eq(googleTokens.userId, req.user!.id))
        .limit(1);
      ...
      async (
        ...
        try {
          ...
          if (contactoId)
            await logActivity(
              "email_sent",
              `Email Gmail: ${email.subject}`,
              "contacto",
              contactoId,
              req.user!.id,
              { via: "google", to: email.to },
            );
          if (oportunidadId)
            await logActivity(
              "email_sent",
    ...
    app.get(
      "/api/google/emails",
      async (
        req: FastifyRequest<{ Querystring: { q?: string; max?: string } }>,
        res,
      ) => {
        try {
          const emails = await getGmailMessages(req.user!.id, {
            q: req.query.q,
            max: parseInt(req.query.max || "25"),
          });
          return { data: emails };
        } catch (err) {
    ...
    app.get(
      ...
      async () => {
        const [c] = await db
          .select()
          .from(googleConfig)
          .where(eq(googleConfig.activo, true))
          .limit(1);
    ...
    app.put(
      ...
      async (req, res) => {
        ...
        if (!body.clientId || !body.redirectUri)
          return res
            .code(400)
            .send({ error: "clientId y redirectUri requeridos" });
        ...
        if (!existing) {
          ...
          const [created] = await db
            ...
            .values({
              ...
              clientSecret: encrypt(body.clientSecret),
              redirectUri: body.redirectUri,
              scopes: body.scopes?.length ? body.scopes : [...GOOGLE_SCOPES],
            })
            .returning();
          const { clientSecret, ...safe } = created;
          return res.code(201).send({ ...safe, clientSecretConfigured: true });
    ...
    app.get(
      ...
      async (
        ...
        const [outlook, google] = await Promise.all([
          getUserCalendar(req.user!.id, { start, end }).catch((e: any) => ({
            error: e?.message || "outlook no disponible",
          })),
          getGoogleCalendar(req.user!.id, {
            timeMin: start,
            timeMax: end,
          }).catch((e: any) => ({
            error: e?.message || "google no disponible",
          })),
        ]);
        const list = [
        ...
        return {
          data: list,
          fuentes: {
            outlook: (outlook as any).error
              ? { connected: false, error: (outlook as any).error }
              : { connected: true },
            google: (google as any).error
              ? { connected: false, error: (google as any).error }
              : { connected: true },
          },
        };
      },
    ...
    app.get("/api/api-tokens", async (req) => {
      return db
        .select({
          id: apiTokens.id,
          nombre: apiTokens.nombre,
          prefijo: apiTokens.prefijo,
          ultimoUsoEn: apiTokens.ultimoUsoEn,
          expiraEn: apiTokens.expiraEn,
          activo: apiTokens.activo,
    ...
    app.post("/api/api-tokens", async (req, res) => {
      const { nombre, expiraEn } = req.body as {
        nombre?: string;
        expiraEn?: string;
      };
      ...
      const { token, record } = await createApiToken(
        req.user!.id,
        nombre,
        expiraEn ? new Date(expiraEn) : undefined,
      );
      return res
        .code(201)
        .send({ id: record.id, token, prefijo: record.prefijo });
    });
    ...
    app.get("/api/webhooks", { preHandler: requireRole("admin") }, async () => {
      ...
      return {
        ...
        endpoints: endpoints.map(({ secret, ...e }) => e),
      };
    });
    ...
    app.post(
      ...
      async (req, res) => {
        ...
        if (!nombre || !url || !secret)
          return res
            .code(400)
            .send({ error: "nombre, url y secret requeridos" });
        try {
          new URL(url);
        } catch {
          return res.code(400).send({ error: "url inválida" });
        }
        const validos = (eventos || []).filter(
          (e) => (WEBHOOK_EVENTS as readonly string[]).includes(e) || e === "*",
        );
        if (validos.length === 0)
          return res.code(400).send({ error: "eventos requeridos" });
        const [ep] = await db
          .insert(webhookEndpoints)
          .values({ nombre, url, secret, eventos: validos })
          .returning();
  ...
  app.register(async (app) => {
    ...
    app.post(
      ...
      async (req, res) => {
        ...
        if (!email || !password || !name)
          return res
            .code(400)
            .send({ error: "email, password y name requeridos" });
        const hash = await hashPassword(password);
        try {
          const [user] = await db
            .insert(users)
            .values({ email, passwordHash: hash, name, role: role || "user" })
            .returning();
    ...
    app.put(
      ...
      async (req, res) => {
        ...
        const [u] = await db
          ...
          .where(eq(users.id, (req as any).params.id))
          .returning();
    ...
    app.delete(
      ...
      async (req, res) => {
        ...
        await db
          .delete(permisos)
          .where(eq(permisos.userId, (req as any).params.id));
    ...
    app.put(
      ...
      { preHandler: requireRole("admin") },
      async (req, res) => {
        const { permisos: lista } = req.body as {
          permisos?: Array<{
            modulo: string;
            r?: boolean;
            w?: boolean;
        ...
        if (!Array.isArray(lista))
          return res.code(400).send({ error: "permisos debe ser un arreglo" });
    ...
    app.get("/api/pipelines", async () =>
      db
        .select()
        .from(pipelines)
        .where(eq(pipelines.activo, true))
        .orderBy(pipelines.nombre),
    );
    app.get(
      "/api/pipelines/:id",
      async (req: FastifyRequest<{ Params: PipelineParams }>, res) => {
    ...
    app.get("/api/pipeline-stages", async () =>
      db.select().from(pipelineStages).orderBy(pipelineStages.orden),
    );
    app.get(
      "/api/pipeline-stages/:id",
      async (req: FastifyRequest<{ Params: PipelineStageParams }>, res) => {
    ...
    // Oportunidades
    const OPP_SORT: Record<string, any> = {
    ...
    app.get(
      "/api/oportunidades",
      async (req: FastifyRequest<{ Querystring: OportunidadesQuery }>) => {
        const {
          limit = "20",
          offset = "0",
          q,
        ...
        if (q)
          conditions.push(
            or(
              ilike(oportunidades.nombre, `%${q}%`),
              ilike(oportunidades.descripcion, `%${q}%`),
            ),
          );
        if (stageId) conditions.push(eq(oportunidades.stageId, stageId));
        if (pipelineId)
          conditions.push(eq(oportunidades.pipelineId, pipelineId));
    ...
    app.get(
      ...
      async (req: FastifyRequest<{ Params: OportunidadParams }>, res) => {
        const [opp] = await db
          .select()
          .from(oportunidades)
          .where(eq(oportunidades.id, req.params.id))
          .limit(1);
    ...
    app.post("/api/oportunidades", async (req, res) => {
      const { custom: customInput, ...rest } =
        req.body as CreateOportunidadBody & { custom?: unknown };
      const body = rest as CreateOportunidadBody;
      const { custom, errors } = await sanitizeCustom(
        "oportunidad",
        customInput,
      );
      if (errors.length)
    ...
    app.put(
      ...
      async (
        req: FastifyRequest<{
          Params: OportunidadParams;
          Body: UpdateOportunidadBody & { custom?: unknown };
        }>,
        res,
      ) => {
        ...
        if (customInput !== undefined) {
          ...
          if (r.errors.length)
            return res.code(400).send({ error: r.errors.join("; ") });
        ...
        if (body.stageId && body.stageId !== oldOpp.stageId) {
          ...
          const [oldStage] = await db
            .select()
            .from(pipelineStages)
            .where(eq(pipelineStages.id, oldOpp.stageId))
            .limit(1);
          await processAutomatizaciones({
            ...
            stage: {
              ...
              esFinal: newStage?.esFinal,
            },
            oldStage: {
              nombre: oldStage?.nombre,
              color: oldStage?.color,
              esFinal: oldStage?.esFinal,
            },
            userId: req.user?.id,
            pipelineId: opp.pipelineId,
            importe: opp.importe,
    ...
    app.delete(
      ...
      async (req, res) => {
        await db
          ...
          .where(eq(oportunidades.id, (req as any).params.id));
        return { ok: true };
      },
    );
    ...
    app.get("/api/oportunidades/export", async (req, res) => {
      ...
      const rows = data.map((o) => ({
        ...
        fechaCierreEstimada: o.fechaCierreEstimada
          ? new Date(o.fechaCierreEstimada).toISOString()
          : "",
      }));
      res.header("Content-Type", "text/csv; charset=utf-8");
      res.header(
        "Content-Disposition",
        'attachment; filename="oportunidades.csv"',
      );
      return res.send(
        toCsv(
          [
    ...
    // Contactos
    const CONTACTO_SORT: Record<string, any> = {
    ...
    app.get(
      "/api/contactos",
      async (req: FastifyRequest<{ Querystring: ContactosQuery }>) => {
        const {
          limit = "20",
          offset = "0",
          q,
    ...
    app.put(
      ...
      async (
        ...
        const [current] = await db
          .select()
          .from(contactos)
          .where(eq(contactos.id, req.params.id))
          .limit(1);
        ...
        const [contacto] = await db
          .update(contactos)
          .set({ ...body, custom, updatedAt: new Date() })
          .where(eq(contactos.id, req.params.id))
          .returning();
        return contacto;
      },
    ...
    app.delete(
      "/api/contactos/:id",
      async (req: FastifyRequest<{ Params: ContactoParams }>, res) => {
    ...
    app.get("/api/contactos/export", async (req, res) => {
      ...
      return res.send(
        toCsv(["nombre", "email", "telefono", "empresa", "cargo"], data),
      );
    });
    ...
    app.post("/api/contactos/import", async (req, res) => {
      ...
      try {
        ...
        return res
          .code(400)
          .send({ error: "Archivo requerido (campo file, multipart)" });
      }
      if (!file)
        return res.code(400).send({ error: "Archivo requerido (campo file)" });
      const { rows } = parseCsv((await file.toBuffer()).toString("utf8"));
      if (rows.length === 0)
        return res.code(400).send({ error: "CSV vacío o sin cabeceras" });
      if (rows.length > 5000)
        return res.code(400).send({ error: "Máximo 5000 filas" });
      let created = 0,
        updated = 0;
      const errors: string[] = [];
      for (let i = 0; i < rows.length; i++) {
        ...
        const line = i + 2;
        if (!r.nombre) {
          errors.push(`Línea ${line}: nombre requerido`);
          continue;
        }
        try {
          if (r.email) {
            const [ex] = await db
              .select()
              .from(contactos)
              .where(eq(contactos.email, r.email))
              .limit(1);
            if (ex) {
              await db
                .update(contactos)
    ...
    app.get(
      ...
      async (
        ...
        const conditions = [];
        if (oportunidadId)
          conditions.push(eq(presupuestos.oportunidadId, oportunidadId));
    ...
    app.get("/api/presupuestos/:id", async (req, res) => {
      const [p] = await db
        .select()
        .from(presupuestos)
        .where(eq(presupuestos.id, (req as any).params.id))
        .limit(1);
      if (!p) return res.code(404).send({ error: "No encontrado" });
      const [opp] = p.oportunidadId
        ? await db
            .select({ id: oportunidades.id, nombre: oportunidades.nombre })
            .from(oportunidades)
            .where(eq(oportunidades.id, p.oportunidadId))
            .limit(1)
        : [];
      const [ct] = p.contactoId
        ? await db
            .select({ id: contactos.id, nombre: contactos.nombre })
            .from(contactos)
            .where(eq(contactos.id, p.contactoId))
            .limit(1)
        : [];
      const [emp] = p.empresaId
        ? await db
            .select({ id: companies.id, nombre: companies.nombre })
            .from(companies)
            .where(eq(companies.id, p.empresaId))
            .limit(1)
        : [];
      return {
        ...p,
        oportunidad: opp || null,
        contacto: ct || null,
        empresa: emp || null,
      };
    });
    ...
    app.post("/api/presupuestos", async (req, res) => {
      ...
      const [p] = await db
        ...
        .values({
          folio,
          oportunidadId: body.oportunidadId || null,
    ...
    // Empresas
    const COMPANY_SORT: Record<string, any> = {
      nombre: companies.nombre,
      createdAt: companies.createdAt,
    };
    app.get(
      "/api/companies",
      async (req: FastifyRequest<{ Querystring: CompaniesQuery }>) => {
        const {
          limit = "20",
          offset = "0",
          q,
        ...
        const conditions = q
          ? or(
              ilike(companies.nombre, `%${q}%`),
              ilike(companies.dominio, `%${q}%`),
            )
          : undefined;
    ...
    app.get(
      "/api/companies/:id",
      async (req: FastifyRequest<{ Params: CompanyParams }>, res) => {
        const [company] = await db
          .select()
          .from(companies)
    ...
    app.get(
      ...
      async (
        req: FastifyRequest<{
          Querystring: { q?: string; limit?: string; soloActivos?: string };
        }>,
      ) => {
        ...
        if (q)
          conditions.push(
            or(
              ilike(productos.nombre, `%${q}%`),
              ilike(productos.sku, `%${q}%`),
            ),
          );
    ...
    app.post("/api/productos", async (req, res) => {
      ...
      if (!body.nombre)
        return res.code(400).send({ error: "nombre requerido" });
      const [p] = await db
        .insert(productos)
        .values({
          nombre: body.nombre,
    ...
    app.put("/api/productos/:id", async (req, res) => {
      ...
      if (body.precio !== undefined)
        body.precio = Math.max(0, Math.round(Number(body.precio) || 0));
      const [p] = await db
        .update(productos)
        .set({ ...body, updatedAt: new Date() })
        .where(eq(productos.id, (req as any).params.id))
        .returning();
      if (!p) return res.code(404).send({ error: "No encontrado" });
      return p;
    });
    ...
    app.get(
      "/api/custom-fields",
      async (req: FastifyRequest<{ Querystring: { entidad?: string } }>) => {
    ...
    app.post(
      ...
      async (req, res) => {
        ...
        if (body.tipo && !(TIPOS as readonly string[]).includes(body.tipo)) {
          return res
            .code(400)
            .send({ error: `tipo debe ser uno de: ${TIPOS.join(", ")}` });
        }
        if (
          body.tipo === "seleccion" &&
          (!body.opciones || body.opciones.length === 0)
        ) {
        ...
        try {
          ...
          return res
            .code(409)
            .send({ error: "Ya existe un campo con esa clave en la entidad" });
        }
      },
    );
    ...
    app.put(
      ...
      async (req, res) => {
        ...
        const [f] = await db
          .update(customFields)
          .set({ ...body, updatedAt: new Date() })
          .where(eq(customFields.id, (req as any).params.id))
          .returning();
    ...
    app.post(
      ...
      async (req: FastifyRequest<{ Params: CompanyParams }>, res) => {
        ...
        if (!contactoId)
          return res.code(400).send({ error: "contactoId requerido" });
        const [company] = await db
          .select()
          .from(companies)
          .where(eq(companies.id, req.params.id))
          .limit(1);
        if (!company)
          return res.code(404).send({ error: "Empresa no encontrada" });
        const [contacto] = await db
          .select()
          .from(contactos)
          ...
          .limit(1);
        if (!contacto)
          return res.code(404).send({ error: "Contacto no encontrado" });
        await db
          .insert(contactCompanies)
          .values({ contactId: contactoId, companyId: req.params.id })
          .onConflictDoNothing();
    ...
    app.delete(
      "/api/companies/:id/contacts/:contactoId",
      async (req: FastifyRequest<{ Params: CompanyContactParams }>, res) => {
        await db
          .delete(contactCompanies)
          .where(
            and(
    ...
    app.get(
      "/api/contactos/:id/companies",
      async (req: FastifyRequest<{ Params: ContactoParams }>) => {
        const rows = await db
    ...
    // Tareas
    const TAREA_SORT: Record<string, any> = {
    ...
    app.post("/api/tareas", async (req, res) => {
      ...
      if (!body.titulo)
        return res.code(400).send({ error: "titulo requerido" });
      const [t] = await db
        .insert(tareas)
        .values({
          titulo: body.titulo,
    ...
    app.get(
      ...
      async (
        ...
        if (!entityType || !entityId)
          return res
            .code(400)
            .send({ error: "entityType y entityId requeridos" });
        return db
          .select()
          .from(archivos)
          .where(
            and(
              eq(archivos.entityType, entityType),
              eq(archivos.entityId, entityId),
            ),
          )
    ...
    app.post("/api/archivos", async (req, res) => {
      ...
      try {
        await storage().putObject(bucketName(), clave, buf, buf.length, {
    ...
    app.get("/api/archivos/:id/download", async (req, res) => {
      const [row] = await db
        .select()
        .from(archivos)
        .where(eq(archivos.id, (req as any).params.id))
        .limit(1);
      ...
      try {
        ...
        return res
          .code(500)
          .send({ error: `Error generando descarga: ${e?.message || e}` });
      }
    });
    ...
    app.delete("/api/archivos/:id", async (req, res) => {
      ...
      if (!row) return res.code(404).send({ error: "No encontrado" });
      try {
        await storage().removeObject(bucketName(), row.clave);
      } catch {
        /* sigue borrando el registro */
      }
    ...
    app.get("/api/equipos", async () => {
      ...
      const members = await db
        .select({
          equipoId: equipoMiembros.equipoId,
          userId: users.id,
          name: users.name,
          email: users.email,
        })
        .from(equipoMiembros)
        .innerJoin(users, eq(equipoMiembros.userId, users.id));
      return list.map((e) => ({
        ...e,
        miembros: members.filter((m) => m.equipoId === e.id),
      }));
    });
    ...
    app.delete(
      ...
      async (req, res) => {
        await db
          ...
          .where(eq(equipoMiembros.equipoId, (req as any).params.id));
    ...
    app.post(
      ...
      async (req, res) => {
        ...
        await db
          ...
          .values({ equipoId: (req as any).params.id, userId })
          .onConflictDoNothing();
        return res.code(201).send({ ok: true });
      },
    );
    ...
    app.delete(
      "/api/equipos/:id/miembros/:userId",
      { preHandler: requireRole("admin") },
      async (req, res) => {
        await db
          .delete(equipoMiembros)
    ...
    async function alcanceIds(
      ...
      if (alcance === "equipo") {
        const mios = await db
          .select()
          .from(equipoMiembros)
          .where(eq(equipoMiembros.userId, userId));
        const ids = mios.map((m) => m.equipoId);
        if (!ids.length) return [userId];
        const mates = await db
          .select()
          .from(equipoMiembros)
          .where(inArray(equipoMiembros.equipoId, ids));
    ...
    app.get(
      ...
      async (req: FastifyRequest<{ Querystring: { entidad?: string } }>) => {
        ...
        if (req.query.entidad)
          conds.push(eq(vistasGuardadas.entidad, req.query.entidad));
        return db
          .select()
          .from(vistasGuardadas)
          .where(and(...conds))
          .orderBy(desc(vistasGuardadas.createdAt));
      },
    );

    app.post("/api/vistas", async (req, res) => {
      const { entidad, nombre, filtros } = req.body as {
        entidad?: string;
        nombre?: string;
        filtros?: unknown;
      };
      if (!entidad || !nombre)
        return res.code(400).send({ error: "entidad y nombre requeridos" });
      const [v] = await db
        .insert(vistasGuardadas)
        .values({
          userId: req.user!.id,
          entidad,
          nombre,
      ...
      async (
        ...
        const [acts, tars, files] = await Promise.all([
          ...
          db
            ...
            .where(
              and(
                eq(tareas.entityType, entityType),
                eq(tareas.entityId, entityId),
              ),
            )
    ...
    app.get("/api/dashboard/stats", async () => {
      const [totalOpps] = await db
        .select({ count: count() })
        .from(oportunidades);
      const [totalValor] = await db
        .select({
          sum: sql<number>`coalesce(sum(${oportunidades.importe}), 0)`,
        })
        .from(oportunidades);
    ...
    // Embudo por etapa: conteo, importe y estimado (importe × probabilidad)
    app.get(
      "/api/reportes/embudo",
      async (req: FastifyRequest<{ Querystring: { pipelineId?: string } }>) => {
        const { pipelineId } = req.query;
        const stageCond = pipelineId
        ...
        const stages = await db
          ...
          .from(pipelineStages)
          .where(stageCond)
          .orderBy(pipelineStages.orden);
        const oppCond = pipelineId
          ? eq(oportunidades.pipelineId, pipelineId)
          : undefined;
        const opps = await db
          .select({
            id: oportunidades.id,
            stageId: oportunidades.stageId,
            importe: oportunidades.importe,
            probabilidad: oportunidades.probabilidad,
          })
          .from(oportunidades)
        ...
        const etapas = stages.map((s) => {
          ...
          const estimado = items.reduce(
            (a, o) => a + ((o.importe || 0) * (o.probabilidad ?? 50)) / 100,
            0,
          );
          return {
            stageId: s.id,
        ...
        const estimadoTotal = etapas.reduce((a, e) => a + e.estimado, 0);
        return {
    ...
    app.get(
      ...
      async (
        req: FastifyRequest<{
          Querystring: { meses?: string; pipelineId?: string };
        }>,
      ) => {
        const meses = Math.min(
          24,
          Math.max(3, parseInt(req.query.meses || "12")),
        );
        const { pipelineId } = req.query;
        const conds = [
          sql`${oportunidades.createdAt} >= date_trunc('month', now()) - (${meses} - 1) * interval '1 month'`,
        ];
        ...
        const map = new Map(
          rows.map((r) => [
            r.mes,
            { count: Number(r.count), importe: Number(r.importe) },
          ]),
        );
        const serie = [];
        const now = new Date();
    ...
    app.get("/api/automatizaciones", async () =>
      db
        ...
        .orderBy(desc(automatizaciones.createdAt)),
    );
    app.post("/api/automatizaciones", async (req, res) => {
      const [auto] = await db
        .insert(automatizaciones)
        .values(req.body as any)
        .returning();
      return res.code(201).send(auto);
    });
    app.put(
      "/api/automatizaciones/:id",
      {
        schema: {
          params: {
            type: "object",
      ...
      async (req, res) => {
        ...
        const [auto] = await db
          ...
          .set({ ...body, updatedAt: new Date() })
          .where(eq(automatizaciones.id, (req as any).params.id))
          .returning();
        return auto;
      },
    );
    ...
    app.get("/api/email-templates", async () =>
      db.select().from(emailTemplates).orderBy(desc(emailTemplates.createdAt)),
    );
    ...
    app.delete(
      "/api/email-templates/:id",
      async (req: FastifyRequest<{ Params: EmailTemplateParams }>, res) => {
        await db
    ...
    // Email campaigns
    app.get("/api/email-campaigns", async () =>
    ...
    app.post(
      ...
      async (req: FastifyRequest<{ Params: EmailCampaignParams }>, res) => {
        const [campaign] = await db
          .select()
          .from(emailCampaigns)
          ...
          .limit(1);
        if (!campaign)
          return res.code(404).send({ error: "Campaña no encontrada" });
        if (!campaign.templateId)
          return res
            .code(400)
            .send({ error: "Campaña sin plantilla asignada" });
        ...
        const [tpl] = await db
          ...
          .limit(1);
        if (!tpl)
          return res.code(400).send({ error: "Plantilla no encontrada" });
    ...
    // Contact Lists
    app.get("/api/contact-lists", async () =>
      db.select().from(contactLists).orderBy(desc(contactLists.createdAt)),
    );
    ...
    app.get(
      ...
      async (req: FastifyRequest<{ Params: ContactListParams }>, res) => {
        const [list] = await db
          .select()
          .from(contactLists)
          .where(eq(contactLists.id, req.params.id))
          .limit(1);
    ...
    app.get(
      "/api/contact-lists/:id/members",
      async (req: FastifyRequest<{ Params: ContactListParams }>) => {
        return db
          .select()
          .from(contactListMembers)
          .where(eq(contactListMembers.listId, req.params.id));
      },
    );
    app.post(
      "/api/contact-lists/:id/members",
      async (req: FastifyRequest<{ Params: ContactListParams }>, res) => {
        const [member] = await db
          ...
          .values({
            ...
            contactoId: (req.body as any).contactoId,
          })
          .returning();
    ...
    app.delete(
      "/api/contact-lists/:id/members/:contactoId",
      async (req, res) => {
        await db
    ...
    app.post("/api/email-subscriptions", async (req, res) => {
      const { contactoId, listaId } = req.body as {
        contactoId: string;
        listaId?: string;
      };
    ...
    app.get(
      ...
      async (
        ...
        const conditions = [];
        if (campaignId)
          conditions.push(eq(emailTracking.campaignId, campaignId));
        if (contactoId)
          conditions.push(eq(emailTracking.contactoId, contactoId));
    ...
    app.get(
      "/api/email-tracking/stats/:campaignId",
      async (req: FastifyRequest<{ Params: { campaignId: string } }>) => {
        ...
        const tracking = await db
          .select()
          .from(emailTracking)
          .where(eq(emailTracking.campaignId, req.params.campaignId));
        const stats = {
          sent: 0,
          opens: 0,
          clicks: 0,
    ...
    app.get(
      ...
      async (
        ...
        return db
          ...
          .where(
            and(
              eq(activities.entityType, "contacto"),
              eq(activities.entityId, req.params.id),
            ),
          )
          .orderBy(desc(activities.createdAt))
          .limit(parseInt(limit));
    ...
    app.get(
      ...
      async (
        ...
        return db
          ...
          .where(
            and(
              eq(activities.entityType, "oportunidad"),
              eq(activities.entityId, req.params.id),
            ),
          )
          .orderBy(desc(activities.createdAt))
