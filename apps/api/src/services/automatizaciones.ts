    ...
    case "send_email": {
      ...
      if (template) {
        const [tpl] = await db
          .select()
          .from(emailTemplates)
          .where(eq(emailTemplates.id, template))
          .limit(1);
        if (tpl) {
          let toEmail = "";
          if (destinatario === "propietario" && context.oportunidadId) {
            const [opp] = await db
              .select({ propietarioId: oportunidades.propietarioId })
              .from(oportunidades)
              .where(eq(oportunidades.id, context.oportunidadId))
              .limit(1);
            if (opp?.propietarioId) {
              const [user] = await db
                .select({ email: users.email, name: users.name })
                .from(users)
                .where(eq(users.id, opp.propietarioId))
                .limit(1);
              toEmail = user?.email || "";
            }
          } else if (destinatario === "contacto" && context.oportunidadId) {
            const [opp] = await db
              .select({ contactoId: oportunidades.contactoId })
              .from(oportunidades)
              .where(eq(oportunidades.id, context.oportunidadId))
              .limit(1);
            if (opp?.contactoId) {
              const [contacto] = await db
                .select({ email: contactos.email })
                .from(contactos)
                .where(eq(contactos.id, opp.contactoId))
                .limit(1);
          ...
          if (toEmail) {
            ...
            await logActivity(
              "email_sent",
              `Email enviado: ${tpl.nombre}`,
              "oportunidad",
              context.oportunidadId || "",
              context.userId,
              { template: tpl.nombre, to: toEmail },
            );
          }
        }
      }
...
export async function processAutomatizaciones(
  context: TriggerContext,
): Promise<void> {
  console.log(
    "[AUTOMATIZACIONES] Processing event:",
    context.evento,
    "for oportunidad:",
    context.oportunidadId,
  );
  ...
  console.log(
    "[AUTOMATIZACIONES] Found",
    autos.length,
    "active automatizaciones for event",
  );
